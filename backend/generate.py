...
from dates import last_month, MonthRange
from datetime import datetime
from pdf import docx_to_pdf
from render import render_invoice, OUTPUT_DIR
...
def build_context(cfg: dict, month: MonthRange, invoice_number: str, timesheet_url: str | None) -> dict:
    preamble = cfg.get("notes_preamble", "").strip()
    if timesheet_url:
        notes = (f"{preamble} Here is the link to the timesheet for the associated "
...
def main() -> int:
    ...
    parser.add_argument("--invoice-number", help="Override invoice number (default: auto-increment).")
    parser.add_argument("--skip-clockify", action="store_true", help="Don't call Clockify.")
    ...
    if not args.skip_clockify:
        try:
            ...
            timesheet_url = create_shared_report(month.iso_start, month.iso_end,
